...
int firVals[10];
int rawXData[WINDOWLEN];
int rawYData[WINDOWLEN];
int rawZData[WINDOWLEN];
...
void APP_Initialize(void) {
    ...
    appData.isMouseReportSendBusy = false;
        ...
        ////Initialization for the original PIC
    ...
    i = 0;
    xOffset = getCoord(data, XXLPOS);
    yOffset = getCoord(data, YXLPOS);
}
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            rawXData[i] = accelX - xOffset;
            rawYData[i] = accelY - yOffset;
            rawZData[i] = accelZ;
            int xVal = fir(rawXData, firVals, i);
            int yVal = fir(rawYData, firVals, i);
            int zVal = fir(rawZData, firVals, i);
            i = (i+1)%WINDOWLEN;
            
            xcurr = getMouseVal(-yVal, zVal);
            ycurr = getMouseVal(xVal, zVal);
            ...
            // every 50th loop, or 20 times per second
                appData.xCoordinate = xcurr;
                appData.yCoordinate = ycurr;
            } else{
